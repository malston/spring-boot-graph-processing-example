...
export SPRING_NEO4J_HOST="$(docker inspect -f '{{ .NetworkSettings.IPAddress }}' graphdb)"
export SPRING_RABBITMQ_HOST="$(docker inspect -f '{{ .NetworkSettings.IPAddress }}' rabbit)"
...
mvn clean test -Dtests=integration -Dserver.host=localhost -Dremote.server.port=$TWITTER_CRAWLER_PORT || die "'mvn clean test -Dtests=integration' failed" 1
